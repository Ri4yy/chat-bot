...
export function SuperAdminCostEstimator({ totalTokens }: { totalTokens: number }) {
  ...
  return (
    <Card className="bg-white dark:bg-zinc-900/50 border-green-500/30 relative group shadow-sm dark:shadow-none">
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        <CardTitle className="text-sm font-medium text-slate-500 dark:text-zinc-400">Оценка затрат</CardTitle>
        ...
        {isEditing ? (
          <div className="mt-2 flex items-center gap-2">
            <Input 
              ...
              className="h-7 text-xs bg-slate-50 dark:bg-zinc-950 border-slate-200 dark:border-zinc-700 w-24 text-slate-900 dark:text-slate-100"
            />
          ...
          <div className="flex items-center justify-between mt-1">
            ...
            <button 
              ...
              className="text-slate-500 dark:text-zinc-500 hover:text-slate-500 dark:text-zinc-500 dark:text-zinc-300 opacity-0 group-hover:opacity-100 transition-opacity"
            >
