  ...
  if (isLoading) {
    return (
      <div className="flex justify-center p-8">
        <RefreshCw className="w-6 h-6 animate-spin text-slate-500 dark:text-zinc-500" />
      </div>
    )
  }
      ...
      <Card className="bg-white dark:bg-[#09090b] shadow-sm dark:shadow-none border-slate-200 dark:border-zinc-800/50">
        <CardHeader className="flex flex-col sm:flex-row sm:items-center justify-between items-start gap-3 pb-2 border-b border-slate-200 dark:border-zinc-800/50 mb-4">
          <div>
            ...
            <CardDescription className="text-slate-500 dark:text-zinc-400 mt-1">О чем чаще всего спрашивают</CardDescription>
          </div>
          <Button 
            variant="outline" 
            size="sm" 
            ...
            className="border-slate-300 dark:border-zinc-700 bg-white dark:bg-zinc-900/50 text-slate-700 dark:text-zinc-300 hover:bg-slate-100 dark:hover:bg-zinc-800"
          >
        ...
        <CardContent>
          {!data?.top_questions || data.top_questions.length === 0 ? (
            <div className="text-center p-8 text-slate-500 dark:text-zinc-500 text-sm border border-dashed border-slate-300 dark:border-zinc-800 rounded-lg">
              Нажмите "AI Анализ", чтобы сгенерировать облако вопросов по истории чатов.
            </div>
          ) : (
            <div className="flex flex-wrap gap-2 mt-2">
              {data.top_questions.map((q, i) => (
                <div key={i} className="px-3 py-1.5 bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/20 text-blue-700 dark:text-blue-200 text-sm rounded-full">
                  {q}
                </div>
              ))}
            </div>
      ...
      <Card className="bg-white dark:bg-[#09090b] shadow-sm dark:shadow-none border-slate-200 dark:border-zinc-800/50">
        <CardHeader className="flex flex-col sm:flex-row sm:items-center justify-between items-start gap-3 pb-2 border-b border-slate-200 dark:border-zinc-800/50 mb-4">
          <div>
            ...
            <CardDescription className="text-slate-500 dark:text-zinc-400 mt-1">На что бот не смог ответить</CardDescription>
          </div>
          {data?.last_updated && (
          ...
          {!data?.unanswered_questions || data.unanswered_questions.length === 0 ? (
            ...
            <ul className="space-y-3 mt-2">
              {data.unanswered_questions.map((q, i) => (
                <li key={i} className="flex gap-3 text-sm text-red-700 dark:text-red-200 bg-red-50 dark:bg-red-950/20 p-3 rounded-lg border border-red-200 dark:border-red-900/30">
