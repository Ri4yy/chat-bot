        ...
        {tab === 'users' ? (
          <>
            <div>
              ...
              <p className="text-slate-500 dark:text-zinc-400 mt-1">Сводка по пользователям и созданным ботам.</p>
            </div>
            ...
            <Card className="bg-white dark:bg-zinc-900/50 shadow-sm dark:shadow-none border-slate-200 dark:border-zinc-800">
              ...
              <CardContent>
                <div className="overflow-x-auto scrollbar-thin scrollbar-thumb-slate-300 dark:scrollbar-thumb-zinc-700 scrollbar-track-transparent pb-2">
                  <table className="w-full text-sm text-left text-slate-500 dark:text-zinc-400">
                    <thead className="text-xs uppercase bg-slate-100 dark:bg-zinc-800/50 text-slate-500 dark:text-zinc-500 dark:text-zinc-300">
                      <tr>
                    ...
                    <tbody>
                      {users?.map((u: any) => (
                        <tr key={u.id} className="border-b border-slate-200 dark:border-zinc-800/50 hover:bg-slate-50 dark:hover:bg-zinc-800/30 transition-colors">
                          <td className="px-4 py-4 font-medium text-slate-700 dark:text-zinc-200">
                            {u.email}
                          </td>
                          ...
                          <td className="px-4 py-4 text-center">
                            {u.is_super_admin ? (
                              ...
                              <span className="text-slate-500 dark:text-zinc-500">Пользователь</span>
                            )}
                          </td>
          ...
          <>
            <div>
              ...
              <p className="text-slate-500 dark:text-zinc-400 mt-2">Детализация использования токенов по каждому созданному проекту.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <Card className="bg-white dark:bg-zinc-900/50 shadow-sm dark:shadow-none border-slate-200 dark:border-zinc-800">
                ...
                <CardContent>
                  <div className="text-3xl font-bold text-slate-900 dark:text-slate-100">{totalChatTokens.toLocaleString()}</div>
                </CardContent>
              </Card>
              ...
              <Card className="bg-white dark:bg-zinc-900/50 shadow-sm dark:shadow-none border-slate-200 dark:border-zinc-800">
                ...
                <CardContent>
                  <div className="text-3xl font-bold text-slate-900 dark:text-slate-100">{totalParseTokens.toLocaleString()}</div>
                </CardContent>
              </Card>

              <Card className="bg-white dark:bg-zinc-900/50 shadow-sm dark:shadow-none border-primary/30 dark:border-primary/30">
                <CardHeader className="flex flex-row items-center justify-between pb-2">
                  <CardTitle className="text-sm font-medium text-slate-500 dark:text-zinc-400">Общий Расход</CardTitle>
              ...
              <CardContent>
                <div className="overflow-x-auto scrollbar-thin scrollbar-thumb-slate-300 dark:scrollbar-thumb-zinc-700 scrollbar-track-transparent pb-2">
                  <table className="w-full text-sm text-left">
                    <thead className="text-xs text-slate-500 dark:text-zinc-400 bg-slate-50 dark:bg-zinc-950/50 uppercase border-b border-slate-200 dark:border-zinc-800">
                      <tr>
                    ...
                    <tbody>
                      {allProjects.map((p) => {
                        ...
                        return (
                          <tr key={p.id} className="border-b border-slate-200 dark:border-zinc-800 hover:bg-slate-50 dark:hover:bg-zinc-800/30 transition-colors">
                            ...
                            <td className="px-4 py-3 font-mono text-xs text-slate-500 dark:text-zinc-500" title={p.user_id}>{p.user_id?.slice(0, 8)}...</td>
