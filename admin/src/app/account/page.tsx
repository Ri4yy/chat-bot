      ...
      <main className="container mx-auto px-4 py-12 relative z-10 max-w-4xl">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Card className="bg-white dark:bg-zinc-900/60 shadow-sm dark:shadow-none border-slate-200 dark:border-zinc-800 backdrop-blur-md md:col-span-1">
            ...
            <CardContent>
              ...
              <div className="space-y-1 mt-4">
                <p className="text-sm text-slate-500 dark:text-zinc-400">ID пользователя</p>
                <div className="flex items-center gap-2">
                  <p className="text-xs font-mono text-slate-500 dark:text-zinc-500 truncate">{user.id}</p>
          ...
          <Card className="bg-white dark:bg-zinc-900/60 shadow-sm dark:shadow-none border-slate-200 dark:border-zinc-800 backdrop-blur-md md:col-span-2">
            <CardHeader>
              <CardTitle className="text-slate-900 dark:text-slate-100 flex items-center gap-2">
                <Activity className="w-5 h-5 text-slate-500 dark:text-zinc-400" /> Статистика использования
              </CardTitle>
              <CardDescription className="text-slate-500 dark:text-zinc-400">Биллинг и ресурсы, потраченные на ваш аккаунт</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="bg-slate-50 dark:bg-zinc-950/50 border border-slate-200 dark:border-zinc-800 rounded-lg p-6 flex items-center gap-6">
                <div className="p-4 bg-primary/10 rounded-full">
                  <BrainCircuit className="w-8 h-8 text-primary" />
                </div>
                <div className="relative group">
                  <h4 className="text-3xl font-bold text-slate-900 dark:text-slate-100 underline decoration-dashed decoration-zinc-700 underline-offset-4 cursor-help">
                  ...
                  {/* CSS Hover Tooltip */}
                  <div className="absolute left-0 top-full mt-3 w-64 p-4 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 shadow-xl dark:shadow-2xl rounded-lg shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                    <div className="flex flex-col gap-3 text-sm">
                      <div className="flex justify-between items-center text-slate-500 dark:text-zinc-300">
                        <span className="flex items-center gap-2"><MessageSquareText className="w-4 h-4 text-slate-500 dark:text-zinc-400" /> Чат-бот:</span>
                        <span className="font-semibold text-slate-900 dark:text-white">{chatTokens.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between items-center text-slate-500 dark:text-zinc-300">
                        <span className="flex items-center gap-2"><Database className="w-4 h-4 text-slate-500 dark:text-zinc-400" /> Парсинг:</span>
                        <span className="font-semibold text-slate-900 dark:text-white">{parseTokens.toLocaleString()}</span>
                      </div>
                    </div>
                    {/* Arrow */}
                    <div className="absolute -top-2 left-6 w-4 h-4 bg-white dark:bg-zinc-900 border-t border-l border-slate-200 dark:border-zinc-700 transform rotate-45"></div>
                  </div>
                </div>
              </div>
              <p className="text-xs text-slate-500 dark:text-zinc-500 mt-4 leading-relaxed">
